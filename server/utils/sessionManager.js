...
class SessionManager {
    ...
    async createSession(userId) {
        ...
        try {
            ...
            // Manejo específico para errores de navegación
            if (error.message.includes('Navigation failed') || 
                error.message.includes('Target closed') ||
                error.message.includes('Protocol error')) {
                logger.warn('Browser navigation error, attempting cleanup', { userId });
                try {
                    await this.destroySession(userId);
                } catch (cleanupError) {
                    logger.error('Error during cleanup after navigation failure', { userId, error: cleanupError.message });
                }
            }
