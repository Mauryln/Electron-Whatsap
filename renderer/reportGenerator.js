...
async function checkSessionAndLoadLabels() {
  ...
  // Verificar que API_URL esté disponible
  if (!window.API_URL) {
    console.warn('API_URL no está disponible aún, esperando...');
    setTimeout(checkSessionAndLoadLabels, 1000);
    return;
  }

  try {
    const response = await fetch(`${window.API_URL}/session-status/${userId}`);
...
async function loadLabels() {
  ...
  try {
    const res = await fetch(`${window.API_URL}/labels/${userId}/all-chats`);
...
async function loadSentMessages() {
  ...
  if (!labelId) return alert("Selecciona una etiqueta");
      ...
      const res = await fetch(`${window.API_URL}/reports/${userId}/${labelId}/messages`);
  const data = await res.json();
